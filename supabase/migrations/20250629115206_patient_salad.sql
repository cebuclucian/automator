...
RETURNS TRIGGER AS $$
BEGIN
  -- Check if the user is the admin and grant unlimited generations
  IF NEW.email = 'admin@automator.ro' THEN
    NEW."generationsRemaining" = 999999; -- A very large number for effectively unlimited generations
  ELSE
    -- Original logic for other users based on plan type
    ...
    ELSIF NEW."planType" = 'enterprise' THEN
      NEW."generationsRemaining" = 20; -- Keep original limit for other enterprise users
    END IF;
  END IF;
...
RETURNS TRIGGER AS $$
BEGIN
  -- Check if the new user is the admin and grant unlimited generations
  IF NEW.email = 'admin@automator.ro' THEN
    INSERT INTO public.users (id, email, "createdAt", "planType", "generationsRemaining")
    VALUES (NEW.id, NEW.email, NEW.created_at, 'free', 999999);
  ELSE
    INSERT INTO public.users (id, email, "createdAt", "planType", "generationsRemaining")
